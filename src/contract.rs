use cosmwasm_std::{Api, Binary, Env, Extern, generic_err, HandleResponse, InitResponse, MigrateResponse, Querier, StdError, StdResult, Storage};

use crate::crypto::{prng};
use crate::msg::{HandleMsg, InitMsg, MigrateMsg, QueryMsg};
use crate::responses::{CreateKeyResponse, SignResponse};
use crate::sign::{pubkey, sign};
use crate::state::{store_key_record, get_key_record, store_seed, get_seed};
use crate::utils::{validate_data_len, authenticate_request, generate_api_key, generate_seed, generate_key_id};
...
pub fn handle<S: Storage, A: Api, Q: Querier>(
    ...
    let result: HandleResponse = match msg {
        HandleMsg::NewKey { key_seed, passphrase } => {
            ...
            let private_key = prng(&seed, key_seed.as_bytes(), 0);

            store_key_record(&mut (deps.storage), &key_id, private_key, &api_key, &passphrase);
        ...
        HandleMsg::Sign { api_key, key_id, data, passphrase } => {
            ...
            if !authenticate_request(&record, &api_key, &passphrase) {
                return Err(StdError::GenericErr { msg: "Unauthorized. Bad API key or passphrase".to_string(), backtrace: None })
            }

            let data_bytes = hex::decode(data).map_err(
                |_|
                    StdError::GenericErr { msg: "Error validating data format: should be hex string".to_string(), backtrace: None })?;

            if !validate_data_len(&data_bytes) {
                return Err(StdError::GenericErr { msg: "Error validating data size: Should be 64 characters".to_string(), backtrace: None })
            }
...
mod tests {
    ...
    use cosmwasm_std::testing::{MOCK_CONTRACT_ADDR, mock_dependencies, mock_env};
